...
### 🟠 B-4. CI가 테스트를 한 번도 돌린 적이 없었음
`package.json`의 `"test": "node --test tests/**/*.test.js"`. Windows에서는 npm이 cmd로 실행해 글롭이 Node에 그대로 전달되고 Node 22가 알아서 확장하니 잘 돌아갔습니다. 그런데 **Linux 러너의 셸은 `globstar`가 꺼져 있어** `**`가 `*`처럼 동작합니다. `tests/*/*.test.js`가 되어 `tests/` 아래 하위 디렉터리를 찾는데 그런 게 없으니 매칭 0건, `Could not find`로 즉시 실패했습니다.
