...
| 팀 | 스페이스, 초대 링크, 역할(owner/admin/member), 실시간 동기화. owner가 멤버를 관리자로 올리면 그 사람도 초대할 수 있습니다 |
...
설명 문안, **권한별 사유**, 데이터 사용 공개 항목, 스크린샷 촬영 순서, 제출 순서를 [`STORE.md`](STORE.md)에 정리해 뒀습니다. 심사는 권한마다 사유를 요구하므로 그 문서를 그대로 쓰면 됩니다.

`npm run check`가 manifest의 권한 목록과 `PRIVACY.md`·`privacy.html`의 설명이 어긋나면 실패시킵니다. 권한을 더하거나 뺄 때는 문서도 같이 고쳐야 통과합니다.
...
`npm test`는 **105개**입니다. 검색(초성·띄어쓰기·태그), 템플릿 변수, 정렬, 바이트 계산, 스토리지 폴백, OAuth 오류 매핑, API 계층(토큰 갱신 재시도·id 인코딩·오류 변환), 테마 시스템(첫 페인트·사용자 선택 우선·시스템 추적), manifest key, analytics 개인정보 불변식, clipboard 읽기·쓰기 규칙, realtime 파싱·재연결, 오류 메시지를 다룹니다.
